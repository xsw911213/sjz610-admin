	...
	export default {
		data() {
			return {
				options: [
					{
						label: '6.09-第五届石家庄旅游交易会启动仪式及巡馆'
					}, {
						label: '6.09-第五届石家庄旅游交易会重点项目发布暨签约仪式'
					}, {
						label: '6.09-《全民导游》颁奖典礼'
					}, {
						label: '6.09-2018中国·石家庄“互联网+旅游”特色小镇峰会'
					}, {
						label: '6.09-市领导会见重要嘉宾及外宾'
					}, {
						label: '6.09-推介餐叙会'
					}, {
						label: '6.09-第二届石家庄市旅游产业发展大会开幕式'
					}, {
						label: '6.09-大型实景剧《遇见·鹿泉》演出'
					}, {
						label: '6.10-旅游项目观摩'
					}, {
						label: '6.11-石家庄市创建全域旅游示范区工作推进会'
					}, {
						label: '6.11-第二届庄里外休闲活动颁奖仪式'
					}
				],
				gridData: [],
        meeting: 0,
        ...
        this.img = `${this.host.imgBaseUrl}${response.key}`;
          ...
          type: 'success'
			...
			submit(){
				if(this.img.length !=0 || this.text.length != 0){
